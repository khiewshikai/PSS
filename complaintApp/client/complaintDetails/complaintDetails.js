...
Template.complaintDetails.events({
    "click .cd-updateBtn": function(e, template){
        ...
        if(originalManagerInstructions === managerInstruction){
            toastr.warning("Please make a change first.");
        }else{
            complaintsCollection.update(
                {_id: complaintMongoID._id},
                {$set: {managerInstruction: managerInstruction}} //need to update timestamp of last edit, but first need to have these attributes at the start when complaints are added. attributes needed: last edited timestamp, last edited user, logging of edits?
            );
